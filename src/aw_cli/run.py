...
def open_mpv(ep_url: str, ep_name: str, progress: int) -> tuple[bool, int]:
    ...
    # Aggiunto encoding='utf-8' e errors='ignore' per risolvere il crash di Windows
    result = subprocess.run(command, shell=True, capture_output=True, text=True, encoding='utf-8', errors='ignore', check=False)
    out = result.stdout or ""

    res = re.findall(r'(\d+):(\d+):(\d+) / [\d:]+ \((\d+)%\)', out)
    if not res:
        # Se MPV si chiude o non dà output, restituisce il progresso salvato finora
...
def setup_config() -> None:
    ...
    if Fzf().run(["sì","no"], "Aggiornare automaticamente la watchlist con AniList? ") == "sì":
        ...
        if ut.os_name == "Darwin":
            ...
            subprocess.run(f"explorer \"{link}\"", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        else:
